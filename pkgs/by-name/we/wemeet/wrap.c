...
// 1. Replicate the memory layout of std::string in a C struct.
// Based on your uprobe script and common GCC/Clang implementations,
// the first two members are a pointer to the character data and the size.
typedef struct {
  const char *ptr;
  size_t size;
  // There is usually a capacity member here as well, but we don't need
  // to access it. Including it can help with struct alignment.
  size_t capacity;
} fake_std_string;

// 2. Define the C function with the correct ABI signature.
//    All std::string references (&& and &) are passed as pointers
//    to the std::string object.
// 3. Use __asm__ to force the symbol name to the exact mangled name.
...
void PrintLog(fake_std_string *tag, fake_std_string *file, int line,
  ...
  // The "%.*s" printf format specifier is perfect here.
  // It prints a specific number of characters from a string,
  // which is exactly what we need for non-null-terminated std::string data.
  printf("-- PrintLog --\n"
         "tag=\"%.*s\"  file=\"%.*s\"  line=%d  func=\"%.*s\"  "
         "level=%d\n"
         "  msg=\"%.*s\"\n",
         (int)tag->size, tag->ptr, (int)file->size, file->ptr, line,
         (int)func->size, func->ptr, level, (int)msg->size, msg->ptr);
}
